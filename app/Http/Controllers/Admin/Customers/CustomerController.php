...
class CustomerController extends Controller
{
    ...
    public function index()
    {
        ...
        if(!empty($user) && $user->user_role == 'vendor'){
            $customers = Order::JOIN('users','users.id','orders.customer_id')->where('orders.user_id',$user->id)->where('user_role','customer')->select('users.*')->paginate(50);
        }
            else{
            $customers = User::where('user_role','customer')->select('users.*')->paginate(50);
